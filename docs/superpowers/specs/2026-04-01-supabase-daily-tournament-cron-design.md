# Tournament Fan-Out Cron Design
...
**Tech Stack:** Supabase Postgres `pg_cron`, `pg_net`, Next.js route handlers, SQL migrations, Vitest

---

## Scope

This change updates the scoring schedule and fan-out behavior. It does not change scoring rules, leaderboard rendering, or tournament roster behavior.

---

## Behavior

1. Each pool stores an IANA timezone.
2. A Supabase cron dispatcher runs hourly in UTC.
3. The dispatcher checks which pools are at local midnight on Thursday through Sunday.
4. The dispatcher triggers scoring for the tournament tied to those pools.
5. The scoring run updates every live pool attached to that tournament.
...
- `README.md` - document the hourly cron setup and required vault secrets.
