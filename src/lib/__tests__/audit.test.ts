...
describe('audit', () => {
  describe('computeScoreDiff', () => {
    ...
    it('detects round id changes', () => {
      const oldScore = createScore('g1', 1, -2)
      const newScore = createScore('g1', 2, -3)
      const diff = computeScoreDiff(oldScore, newScore)
      expect(diff.changed).toBe(true)
    ...
    it('detects status changes', () => {
      const oldScore = createScore('g1', 1, -2, 'active')
      const newScore = createScore('g1', 1, -2, 'withdrawn')
      const diff = computeScoreDiff(oldScore, newScore)
      expect(diff.changed).toBe(true)
    ...
    it('detects birdie count changes', () => {
      const oldScore = createScore('g1', 1, -2, 'active', 1)
      const newScore = createScore('g1', 1, -2, 'active', 2)
      const diff = computeScoreDiff(oldScore, newScore)
      expect(diff.changed).toBe(true)
  ...
  describe('buildRefreshAuditDetails', () => {
    it('summarizes diffs across multiple golfers', () => {
      const oldScores = new Map<string, TournamentScore>([
        ['g1', createScore('g1', 1, -2)],
        ['g2', createScore('g2', 1, 0)],
      ])
