...
type MemberPool = Pick<
  Pool,
  'id' | 'name' | 'tournament_name' | 'status' | 'deadline' | 'timezone' | 'picks_per_entry'
>
...
> {
  const { data: memberRows, error: membersError } = await supabase
    .from('pool_members')
    .select('pool_id, role, pools(id, name, tournament_name, status, deadline, timezone, picks_per_entry)')
    .eq('user_id', userId)
