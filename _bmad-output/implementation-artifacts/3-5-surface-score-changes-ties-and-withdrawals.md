...
As a commissioner,
I want to surface score changes, ties, and withdrawals,
so that standings remain fair and understandable.

## Acceptance Criteria

1. Given tournament results change, when the system recalculates standings, then score changes are reflected in the leaderboard, and ties are resolved using the configured rules.
2. Given a golfer withdraws, when scoring is recalculated, then the withdrawal is handled according to the scoring rules, and the result is visible in standings.
...
- Story source: [Source: _bmad-output/planning-artifacts/epics.md#Story 3.5]
...
- _bmad-output/implementation-artifacts/3-5-surface-score-changes-ties-and-withdrawals.md
