...
As a commissioner,
I want to reuse the pool for a future tournament,
so that I do not need to recreate the same configuration from scratch.

## Acceptance Criteria

1. Given I have a previous pool configuration, when I start a new tournament from that pool, then the system copies the reusable pool settings, and it creates a fresh event instance for the new tournament.
2. Given the old pool contains finished entries or standings, when I reuse the pool, then historical results remain separate from the new event, and the new event starts cleanly.
...
- Story source: [Source: _bmad-output/planning-artifacts/epics.md#Story 1.6]
...
- _bmad-output/implementation-artifacts/1-6-reuse-the-pool-for-a-future-tournament.md
