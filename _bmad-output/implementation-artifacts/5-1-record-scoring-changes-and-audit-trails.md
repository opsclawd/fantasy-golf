...
As a commissioner,
I want to record scoring changes and audit trails,
so that the pool can be reviewed later if something looks wrong.

## Acceptance Criteria

1. Given scores are updated, when the system stores the update, then it records what changed and when, and the change is available for later review.
...
- _bmad-output/implementation-artifacts/5-1-record-scoring-changes-and-audit-trails.md
